        ...
        foreach ($this->_fields as $name => $field) {
            ...
            if ( CRM_Utils_Array::value( $name, $this->_paramLookup ) ||
                ...
                if ( $cfID ) {
					...
					if($name == 'employer_street_address'){
						$this->_select[$name]	= "emp_add.street_address as employer_street_address";
					}
					else if($name == 'employer_supplemental_address1'){
						$this->_select[$name]	= "emp_add.supplemental_address_1 as employer_supplemental_address1";
					}
					else if($name == 'employer_supplemental_address2'){
						$this->_select[$name]	= "emp_add.supplemental_address_2 as employer_supplemental_address2";
					}
					else if($name == 'employer_city'){
						$this->_select[$name]	= "emp_add.city as employer_city";
					}
					else if($name == 'employer_postal_code'){
						$this->_select[$name]	= "emp_add.postal_code as employer_postal_code";
					}
					else if($name == 'employer_postal_code_suffix'){
						$this->_select[$name]	= "emp_add.postal_code_suffix as employer_postal_code_suffix";
					}
					/*else if($name == 'employer_state'){
						$this->_select[$name]	= "emp_state.name as employer_state";
					}*/
					else if($name == 'employer_country'){
						$this->_select[$name]	= "emp_country.name as employer_country";
					}
					else if($name == 'employer_county'){
						$this->_select[$name]	= "emp_state.name as employer_county";
					}
    ...
    static function fromClause( &$tables , $inner = null, $right = null, $primaryLocation = true, $mode = 1 ) 
    {
        ...
        foreach ( $tables as $name => $value ) {
            ...
            case 'civicrm_address':
                if ( $primaryLocation ) {
		      $from .= " $side JOIN civicrm_address emp_add ON ( contact_a.employer_id = emp_add.contact_id AND emp_add.is_primary = 1 )";					
                    $from .= " $side JOIN civicrm_address ON ( contact_a.id = civicrm_address.contact_id AND civicrm_address.is_primary = 1 )";
                } else {
		      $from .= " $side JOIN civicrm_address emp_add ON ( contact_a.employer_id = emp_add.contact_id ) ";	
            ...
            case 'civicrm_state_province':
		  $from .= " $side JOIN civicrm_state_province emp_state ON emp_add.state_province_id = emp_state.id ";

                $from .= " $side JOIN civicrm_state_province ON civicrm_address.state_province_id = civicrm_state_province.id ";
                continue;

            case 'civicrm_country':
                $from .= " $side JOIN civicrm_country emp_country ON emp_add.country_id = emp_country.id ";
            ...
            case 'civicrm_county':
		  $from .= " $side JOIN civicrm_county emp_county ON emp_add.county_id = emp_county.id ";
