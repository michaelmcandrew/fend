...
class CRM_Contact_Selector extends CRM_Core_Selector_Base implements CRM_Core_Selector_API 
{
    ...
    function &getRows($action, $offset, $rowCount, $sort, $output = null) {
           ...
           // echo "<pre>"; print_r($result); echo '</pre><br />';
            ...
            foreach ($names as $property) {
				...
				if ( $cfID = CRM_Core_BAO_CustomField::getKeyID($property)) {
					//echo "<pre>"; print_r($this->_options); echo '</pre><br />';
            ...
            if ( $output != CRM_Core_Selector_Controller::EXPORT ) {
				...
				// getting membership no
				require_once('CRM/Core/BAO/CustomValueTable.php');
				$params = array( 'entityID' => $result->contact_id, 'custom_100' => 1);

				$custom_result = CRM_Core_BAO_CustomValueTable::getValues($params);
				
				if(!$custom_result['is_error'])
					$row['membership_no_100'   ] = $custom_result['custom_100'];
				else
					$row['membership_no_100'   ] = '';
				#### end ####### by jyoti : 15jul10 #################################
            }
            // Dedupe contacts        
            if ( ! $empty ) {
		...
		//echo 'hello<pre>'; print_r($rows); echo '</pre>'; exit;
