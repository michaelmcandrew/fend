    ...
    static function sendMail( $contactID, &$values, $participantId, $isTest = false, $returnMessageText = false )
    {
        ...
        if ( $values['event']['is_email_confirm'] ) {
            ...
            if ( isset( $email ) ) {
                ...
                // Was here, moved for multilanguage text
                // $template->assign( 'confirm_email_text', CRM_Utils_Array::value( 'confirm_email_text', $values['event'] ) );
                
                // MTL Added section
                // Need to get the language specific bit of text
                // For now this is hard coded but can sort out later
                // Only do this for conference event
                if ($values['event']['event_type_id'] == "1")
                {
                   // First establish the language
                   global $tsLocale;
                   ...
                   switch ( $tsLocale ) { 
                     case "en_GB":
                        ...
                        break;
                     case "es_ES":
                        $template->assign( 'confirm_email_text', 'Spanish Thank you text' );
                        break;
                     default:
                        break;
                   }
        ...
        CRM_Core_Error::debug_log_message( 'Send email Event.php values[event][event_type_id]= '.$values['event']['event_type_id'], $out = false );

        // MTL Need to send an email to the booking email address with the same details if the delegate was the primary delegate
        // MTL First find out if the event being booked is the conference event, all event types of 1 are conference events
        if ($values['event']['event_type_id'] == "1")
        {
          $isPrimary = $template->get_template_vars('isPrimary');

          // Only do this if the person if primary participant
          if ($isPrimary == "1")
          {

                $sql = "
SELECT
civicrm_value_conference_delegate_9.booking_contact_email_address_44
FROM
civicrm_value_conference_delegate_9
WHERE
civicrm_value_conference_delegate_9.entity_id = %1
";

            $params = array( 1 => array( $participantId, 'Integer' ) );
            $dao = CRM_Core_DAO::executeQuery( $sql, $params );

            while ( $dao->fetch( ) ) {
              $bookingEmailAddress    = $dao->booking_contact_email_address_44;
            }

            CRM_Core_Error::debug_log_message( 'Send email Event.php isPrimary= '.$isPrimary, $out = false );
            CRM_Core_Error::debug_log_message( 'Send email Event.php participantId= '.$participantId, $out = false );
            CRM_Core_Error::debug_log_message( 'Send email Event.php bookingEmailAddress = '.$bookingEmailAddress, $out = false );
            CRM_Core_Error::debug_log_message( 'Send email Event.php email= '.$email, $out = false );
            // print "<pre>bookingEmailAddress=".$bookingEmailAddress;
            // print_r ($customPreArr);
            // print_r ($values);
            // print "</pre>";
            // die;

            // Ensure the email address is differnt, otherwise person will receive two emails
            if ($bookingEmailAddress != $email)
            {
              ...
              // If we did send a full email to the booking email address then we dont want the primary contact getting all that too
              // So we set the relevant values to false, so they will only see their details

              // This code doesn't quite work at the moment
              // The amounts shown in the first delegates email are not shown correctly
              // Everyone else's emails work ok
              // Maybe this needs help from Core team?

              // $template->assign ( 'isPrimary' , 0 );
              // $template->assign( 'customProfile', null );

              // Need to get the message again as it should have less information
