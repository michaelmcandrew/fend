...
class CRM_Event_BAO_Query 
{
    ...
    static function whereClauseSingle( &$values, &$query ) 
    {
        ...
        // Cases added by MTL to allow query on registered by ID
        // Wasn't able to query on the registered by ID
        case 'participant_registered_by_id':
            $query->_where[$grouping][] = "civicrm_participant.registered_by_id $op $value";
            $query->_tables['civicrm_participant'] = $query->_whereTables['civicrm_participant'] = 1;
            return;
    ...
    static function buildSearchForm( &$form ) 
    {
        ...
        // Added by MTL to ensure the API's work when it comes to passing in the registered by ID
        $form->addElement( 'text', 'participant_registered_by_id', ts( 'Registered by ID' ), array( 'size' => 8, 'maxlength' => 8 ) );
