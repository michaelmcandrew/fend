...
{if ! empty( $fields )}
   ...
   {if $help_pre && $action neq 4}<div class="messages help">{$help_pre}</div>{/if} 
    ...
    {* Social companion amount for diaply in help only *}
    {if $config->defaultCurrency eq 'EUR'}
    {assign var="socialCompFee" value='125'|crmMoney}
    {else}
    {assign var="socialCompFee" value='110'|crmMoney}
    {/if}
    {* Default Currency = {$config->defaultCurrency} *}
    {* socialCompFee = {$socialCompFee} *}
  ...
  {if isset($emailfield.error)}
      <span class="error upper">
  {/if}         
            <label for="email-5">{ts}Delegate's email address{/ts}
  {if isset($emailfield.error)}
      </span>
  {/if}                
        ...
        <td class="view-value">
  {if isset($emailfield.error)}
      {$emailfield.html|crmReplace:class:'form-text huge required error'}
  {else}
      {$emailfield.html|crmReplace:class:'form-text huge'}
  {/if} 
        ...
        <tr><td>{$form.$n.html}{* quickform add closing </td> </tr>*}
         ...
         {else}
           ...
           {if ($fieldFirstSix == "custom") and ($form.$n.type == "text")}
             {* <pre>x{$form.n.error}x</pre> *}
             {if isset($form.$n.error) }
                 {$form.$n.html|crmReplace:class:'form-text huge required error'}
             {else}
                 {$form.$n.html|crmReplace:class:'form-text huge'}
             {/if}
           {else}
             {$form.$n.html}
           {/if}
       ...
       {include file="CRM/Custom/Form/AutoComplete.tpl" element_name = $n }
        ...
        {include file="CRM/common/calendar/body.tpl" dateVar=$form.$n.name startDate=1905 endDate=2010 doTime=1  trigger="$form.$n.name"}
         ...
         {if $field.help_post && $action neq 4 && $action neq 1028}
          ...
          {* Need to replace the amount with the euro amount for social companion *}
          <br /><span class="description">
          {if $fieldName eq "custom_31"}
              {* Look for '#amount#' and replace according to whatever amount we want in the formatted currency *}
              {ts}{$field.help_post|replace:'#amount#':$socialCompFee}{/ts}
          {else}
              {ts}{$field.help_post}{/ts}
          {/if}
      </span> 
         {/if} 
       </td>
