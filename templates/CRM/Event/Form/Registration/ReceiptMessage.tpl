...
{/if}
{else}
{ts}Please print this confirmation for your records.
{/ts}
{/if}
===========================================================
{ts}Conference Information and Location{/ts}
...
{* {if $email} *}
{if $customPre.custom_44}
===========================================================
{ts}Booking Email{/ts}

===========================================================
{* {$email} *}
{$customPre.custom_44.html}
{/if}
...
{/if}
{if $amount && !$lineItem} 
{foreach from=$amount item=amount key=level}{$amount.amount|crmMoney} {$amount.label|strip_tags:false}	

{/foreach}
{/if}
{if $isPrimary }
{ts}Total Amount{/ts}     : {$totalAmount|crmMoney} {if $hookDiscount.message}({$hookDiscount.message})
