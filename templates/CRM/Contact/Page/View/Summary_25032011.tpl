            ...
            {if $hookContentPlacement neq 3}
                ...
                <div id="contactTopBar" class="ui-corner-all">
                    <table>
          		    ...
          		    {crmAPI entity='contact' action="get" var="employerDetails" contact_id=$employer_id}
                            <td class="label">{ts}Employer Telephone No{/ts}</td><td>{$employerDetails.$employer_id.phone}</td>
                            <td class="label">{ts}Employer Email Address{/ts}</td><td>{$employerDetails.$employer_id.email}</td>
                            ...
                            <table>
                                {foreach from=$email item=item }
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                        {* MTL Added section to display if the email is on hold *{
                                        {* <span class={if $privacy.do_not_email}"do-not-email" title="Privacy flag: Do Not Email" {elseif $item.is_primary eq 1}"primary"{/if}><a href="mailto:{$item.email}">{$item.email}</a>{if $item.is_bulkmail}&nbsp;(Bulk){/if}</span> *}
                                        <span class={if ($privacy.do_not_email) or ($item.on_hold eq "1") }"do-not-email" title={if ($privacy.do_not_email) }"Privacy flag: Do Not Email"{else}"On Hold"{/if} {elseif $item.is_primary eq 1}"primary"{/if}><a href="mailto:{$item.email}">{$item.email}</a>{if $item.is_bulkmail}&nbsp;(Bulk){/if}</span>                                        
                                        </td>
                                    </tr>
                                    {/if}
                                {/foreach}
						 ...
						 <table>
							<tr>
								...
								<td>{$addressee_display}</td>
                                                        ...
                                                        <tr>
								<td class="label">{ts}Group{/ts}{if $addressee_custom}<br/><span style="font-size:8px;">({ts}Customized{/ts})</span>{/if}</td>
								<td>
                                                        {crmAPI entity="participant" action="get" var="participants" contact_id=$contactId}

							{*<pre> {$participants|print_r} </pre>*}
							 {$participants.custom_90}
                                                         </td>
							</tr>
						 </table>
						</div>
